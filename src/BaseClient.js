...
export class BaseClient {
  constructor(apiKey) {
    if (!apiKey) {
      throw new Error('MailAPI key is not configured. Please pass the API key to the MailApiDev constructor.');
    }
  ...
  async request(method, path, payload) {
    try {
      ...
      if (err.response) {
        ...
        // Network error
        return { data: null, error: new MailApiError('Network error, could not reach MailAPI.', 'network_error') };
      } else {
        // Other JavaScript error
        return { data: null, error: new MailApiError(err.message, 'internal_error') };
      }
    }
  }
}